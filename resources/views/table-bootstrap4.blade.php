    ...
    <div class="position-relative">
        ...
        <div class="d-block d-md-none mt-3">
            ...
            @if (count($filterColumn))
                <div class="mb-3">
                    <button
                        class="btn btn-primary btn-sm btn-block mb-2 d-flex justify-content-center align-items-center shadow-sm"
                        type="button" data-toggle="collapse" data-target="#mobileFilters-{{ $this->getId() }}"
                        aria-expanded="false" aria-controls="mobileFilters-{{ $this->getId() }}">
                            ...
                            @foreach ($columns as $index => $col)
                                @if (isset($filterColumn[$index]))
                                    <div wire:key="filter-mobile-{{ $index }}" class="mb-3">
                                        <label
                                            class="text-muted small font-weight-bold mb-1 d-block">{!! strip_tags($col['name']) !!}</label>
                                        @include('lara-pack.livewire-datatable::filter-bootstrap4', [
                                            'index' => $index,
                                            'filter' => $filterColumn[$index],
                                            'keyPrefix' => 'mobile',
                        ...
                        @foreach ($regularColumns as $col)
                            ...
                            <div class="border-bottom py-2">
                                <label class="font-weight-bold text-muted small text-uppercase mb-1 d-block"
                                    style="font-size: 0.7rem; letter-spacing: 0.05em;">{!! $col['name'] !!}</label>
                                <div class="text-wrap {!! str_replace("class='", '', str_replace("'", '', $cell_class)) !!}" {!! $cell_style !!}
